...
/**
 * Called before executing the first TestRunner.
 *
 * Allows the reporter to setup any initial state (which must be global).
 */
void longBowReportTesting_Initialize(void);

/**
 * Called after finished with the last TestRunner.
 *
 * Allows the reporter to cleanup any (global) state.
 */
void longBowReportTesting_Finalize(void);
